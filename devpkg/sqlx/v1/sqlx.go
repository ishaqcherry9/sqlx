...
import (
	...
	"github.com/ishaqcherry9/sqlx/internal/structs"
)
...
func NewModel(g genx.Context, t types.Type) *Model {
	...
	if p := g.PackageByPath("github.com/ishaqcherry9/sqlx/pkg/types"); p != nil {
...
`)
	...
	return s.Template(
		code,
		s.Arg(ctx, "T", m.Ident()),
		s.Arg(ctx, "builder.Eq", s.ExposeUnsafe(ctx, "github.com/ishaqcherry9/sqlx/pkg/builder", "Eq")),
	)
}
